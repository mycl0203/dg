...
select (`cat`.`name` collate utf8_tolower_ci)          AS `VIEW_CATALOG`,
       (`sch`.`name` collate utf8_tolower_ci)          AS `VIEW_SCHEMA`,
       (`vw`.`name` collate utf8_tolower_ci)           AS `VIEW_NAME`,
       (`vtu`.`table_catalog` collate utf8_tolower_ci) AS `TABLE_CATALOG`,
       (`vtu`.`table_schema` collate utf8_tolower_ci)  AS `TABLE_SCHEMA`,
       (`vtu`.`table_name` collate utf8_tolower_ci)    AS `TABLE_NAME`

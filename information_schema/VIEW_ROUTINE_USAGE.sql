...
select (`cat`.`name` collate utf8_tolower_ci)            AS `TABLE_CATALOG`,
       (`sch`.`name` collate utf8_tolower_ci)            AS `TABLE_SCHEMA`,
       (`vw`.`name` collate utf8_tolower_ci)             AS `TABLE_NAME`,
       (`vru`.`routine_catalog` collate utf8_tolower_ci) AS `SPECIFIC_CATALOG`,
       (`vru`.`routine_schema` collate utf8_tolower_ci)  AS `SPECIFIC_SCHEMA`,

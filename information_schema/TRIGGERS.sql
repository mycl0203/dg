...
select (`cat`.`name` collate utf8_tolower_ci) AS `TRIGGER_CATALOG`,
       (`sch`.`name` collate utf8_tolower_ci) AS `TRIGGER_SCHEMA`,
       `trg`.`name`                           AS `TRIGGER_NAME`,
       `trg`.`event_type`                     AS `EVENT_MANIPULATION`,
       (`cat`.`name` collate utf8_tolower_ci) AS `EVENT_OBJECT_CATALOG`,
       (`sch`.`name` collate utf8_tolower_ci) AS `EVENT_OBJECT_SCHEMA`,
       (`tbl`.`name` collate utf8_tolower_ci) AS `EVENT_OBJECT_TABLE`,
